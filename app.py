...
# Берем твои пароли и URL из секретов
ADMIN_PASSWORD = st.secrets["ADMIN_PASSWORD"]
STAFF_PASSWORD = "staff123" # Пароль для помощника
DB_URL = st.secrets["DB_URL"]
engine = create_engine(DB_URL)

# Авторизация (встроена в начало твоего интерфейса)
...
# Создание таблицы для документов (если нет)
...
# --- ТВОЙ ОРИГИНАЛЬНЫЙ ВИЗУАЛ ---
st.sidebar.info(f"Аккаунт: {st.session_state.role}")
if st.sidebar.button("Выйти"):
    st.session_state.auth = False
    st.rerun()

# Твои вкладки из первого файла
...
with tab1:
    ...
    # Помощник только смотрит, админ может править
    if st.session_state.role == "admin":
        edited_df = st.data_editor(df, num_rows="dynamic", key="editor_clients")
        if st.button("Сохранить изменения в базе"):
            # Тут твоя логика сохранения
...
with tab4:
    ...
    if not cls.empty:
        ...
        st.divider()
        # НОВОЕ: Блок для договоров
        st.write("### 📄 Договоры и документы")
        col_in, col_ls = st.columns([1, 2])
        with col_in:
            d_name = st.text_input("Название (например: Договор поставки)")
            d_link = st.text_input("Ссылка на файл (Google Drive)")
            if st.button("Прикрепить ссылку"):
                with engine.connect() as conn:
                    conn.execute(text("INSERT INTO client_documents (client_id, file_name, file_url) VALUES (:i, :n, :l)"), {"i": cid, "n": d_name, "l": d_link})
                    conn.commit()
                st.rerun()
        with col_ls:
            with engine.connect() as conn:
...
with tab5:
    if st.session_state.role == "admin":
        ...
        # Твои графики
        with engine.connect() as conn:
            df_g = pd.read_sql("SELECT category, amount FROM expenses", conn)
        if not df_g.empty:
